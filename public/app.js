...
fetch('/api/audit', { method: 'POST' })
  ...
  .then(({ ok, j }) => {
    ...
    const diff = j.claims.length !== demoSnap.claims.length ? ` · 저장본 ${demoSnap.claims.length}건` : ''
    live.textContent = `${j.cached ? '방금 감사' : '지금 감사'} · ${ran} · ${j.claims.length}건 · ${(j.ms / 1000).toFixed(1)}초${diff}`
