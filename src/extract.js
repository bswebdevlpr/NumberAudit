 ...
 * 문서당 1회 구조는 하루 20문서까지만 읽을 수 있고, 실제 프로젝트는 글이 수백 건이다.
 * **한도가 구조를 정했다.**
 *
...
async function extractOneBatch(docs, depth = 0, chain, deadline) {
  ...
  try {
    ...
    if (e.code === 'MAX_TOKENS' && docs.length > 1 && depth < 4) {
      ...
      return [...await extractOneBatch(docs.slice(0, mid), depth + 1, chain, deadline),
              ...await extractOneBatch(docs.slice(mid), depth + 1, chain, deadline)]
    }
    throw e
  }
}
 ...
 */
export async function extractClaims(docs, { chain, deadline } = {}) {
  const byId = new Map(docs.map((d) => [d.docId, d]))
  const batches = planBatches(docs)

  const raw = []
  for (const b of batches) raw.push(...await extractOneBatch(b, 0, chain, deadline))
