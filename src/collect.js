 ...
 *
 * 실측(이 데모 프로젝트의 에디터 글): 표 한 개가 `outContent` 에서 `"항목값코드리뷰 평균 대기4시간배포3회"`
...
/**
 * ⚠️ **정규식으로 HTML 을 판다. 이게 이 파일의 알려진 약점이다.**
 *
 * 쓸 수 있는 근거 — 파싱 대상이 **플로우 에디터가 만든 HTML 한 종류**다. 임의의 웹 문서가 아니다.
 * 그리고 여기서 깨져도 **게이트가 뒤에서 받는다**: 본문이 어긋나면 인용이 원문과 안 맞아 폐기된다.
 * 조용히 틀린 답이 나오는 구조가 아니라, 틀리면 **버려지는** 구조다.
 *
 * 깨지는 조건 — 중첩 표, 속성값 안의 `>`, 주석 안의 태그 모양 문자열.
 * 실측 범위는 에디터 글 3건 · 이음매 22곳 · 표 1개다. 그 밖은 안 쟀다.
 *
 * 🔑 **파서로 갈아탈 조건**: 플로우 밖에서 온 HTML 을 받게 되거나, 중첩 표가 실제로 나오거나,
 *    위 조건에서 깨지는 게 측정되면. 그때는 의존성 하나를 늘리는 편이 싸다.
 *    지금은 입력이 한정돼 있고 실측으로 확인됐으므로 안 늘린다.
 */
